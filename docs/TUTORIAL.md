...
```powershell
.\dns-cli.cmd generate all --profile mame --resolvers testdata\resolvers_sample.json --limit 10 --no-dmvpn
```
...
.\dns-cli.cmd pipeline run --input testdata\dns_sample.txt --profile mame --preset low --skip-slipnet
```
